...
def categorical_sample(probs, use_cuda=False):
    int_acs = torch.multinomial(probs, 1)  # Returns a tensor where each row contains :attr:`num_samples` indices
    # sampled from the multinomial probability distribution located in the corresponding row of tensor :attr:`input`.
    #  ;;Returns indexes of the value sampled from each row of the input(arg1)
    if use_cuda:
    ...
    return int_acs, acs  # indexes of values and one-hot of indexes of values[MP for one-hot]
