...
class AttentionCritic(nn.Module):
    ...
    """
    def __init__(self, sa_sizes, hidden_dim=32, norm_in=True, attend_heads=1, critic_type=None):
        """
        Inputs:
    ...
    def forward(self, inps, agents=None, return_q=True, return_all_q=False,
        ...
        for i, a_i in enumerate(agents):
            ...
            # pick attention weights for current agent
            attention_weights = [probs.squeeze(1) for probs in all_attend_probs[i]]
            # mean over batch across all the other agents for each head
            mean_att_wgts = [atten_heads.mean(dim=0) for i, atten_heads in enumerate(attention_weights)]
            mean_att_wgts_per_agent = []
            for h_idx in range(self.attend_heads):
                mean_att_wgts_per_agent.append([mean_att_wgts[h_idx][i] for i in range(self.nagents-1)])
            mean_att_wgts_per_agent = np.mean(np.asarray(mean_att_wgts_per_agent),0)

            if logger is not None:
                logger.add_scalars(self.critic_type+'/agent%i/attention_weights with mean over attention_heads' % a_i,
                                       dict(('agent%i_weight' % ag_n, wgt)
                                            for ag_n, wgt in enumerate(mean_att_wgts_per_agent)), niter)
            if return_q:
                agent_rets.append(q)
            if return_all_q:
            ...
            if logger is not None and self.critic_type == 'Distance_critic':
                logger.add_scalars(self.critic_type+'/agent%i/attention' % a_i,
