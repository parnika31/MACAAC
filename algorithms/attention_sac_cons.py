...
class AttentionSAC(object):
    ...
    def __init__(self, agent_init_params, sa_size,
                 ...
                 **kwargs):
        """
        Updating for 2 penalty critics here.
        ...
        self.penalty_critic = AttentionCritic(sa_size, hidden_dim=critic_hidden_dim,
                                      attend_heads=attend_heads, critic_type= 'Collision_Critic_Coll')
        ...
        self.penalty_critic_optimizer = Adam(self.penalty_critic.parameters(), lr=q_lr,
                                     weight_decay=1e-3)
        self.penalty2_critic = AttentionCritic(sa_size, hidden_dim=critic_hidden_dim,
                                              attend_heads=attend_heads, critic_type= 'Collision_Critic_Dep')
        self.penalty2_target_critic = AttentionCritic(sa_size, hidden_dim=critic_hidden_dim,
                                                     attend_heads=attend_heads)
        hard_update(self.penalty2_target_critic, self.penalty2_critic)
        self.penalty2_critic_optimizer = Adam(self.penalty2_critic.parameters(), lr=q_lr,
                                             weight_decay=1e-3)
        ...
        self.pen_trgt_critic_dev = 'cpu'
        self.nw_pen_critic_dev = 'cpu'
        self.nw_pen_trgt_critic_dev = 'cpu'
        self.niter = 0
    ...
    def update_penalty_critic(self, sample, soft=True, logger=None, **kwargs):
        ...
        nw_next_qs = self.penalty2_target_critic(pen_trgt_critic_in)
        nw_critic_rets = self.penalty2_critic(pen_critic_in, regularize=True,
                                          logger=logger, niter=self.niter)
        pen2_q_loss = 0
        for a_i, nq, log_pi, (pq, regs) in zip(range(self.nagents), nw_next_qs,
                                               next_log_pis, nw_critic_rets):
            target_q = (penalties_2[a_i].view(-1, 1) +
                        self.gamma * nq *
                        (1 - dones[a_i].view(-1, 1)))
            if soft:
                target_q -= log_pi / self.reward_scale
            pen2_q_loss += MSELoss(pq, target_q.detach())
            for reg in regs:
        ...
        self.penalty2_critic_optimizer.step()
        self.penalty2_critic_optimizer.zero_grad()
    ...
    def update_all_targets(self):
        ...
        soft_update(self.penalty2_target_critic, self.penalty2_critic, self.tau)
    ...
    def prep_training(self, device='gpu'):
        ...
        self.penalty2_critic.train()
        self.penalty2_target_critic.train()
        for a in self.agents:
            a.policy.train()
        ...
        if not self.nw_pen_critic_dev == device:
            self.penalty2_critic = fn(self.penalty2_critic)
            self.nw_pen_critic_dev = device
        ...
        if not self.nw_pen_trgt_critic_dev == device:
            self.penalty2_target_critic = fn(self.penalty2_target_critic)
            self.nw_pen_trgt_critic_dev = device
    ...
    def save(self, filename):
        ...
        save_dict = {'init_dict': self.init_dict,
                     ...
                     'penalty_critic_new_params': {'penalty_critic_new': self.penalty2_critic.state_dict(),
                                                   'target_critic_new': self.penalty2_target_critic.state_dict(),
                                                   'penalty_critic_new_optimizer': self.penalty2_critic_optimizer.state_dict()}
                     }
    ...
    def init_from_save(cls, filename, load_critic=False, type ='constrained'):
        ...
        if load_critic:
            ...
            if type == 'constrained':
                ...
                penalty_critic_new_params = save_dict['penalty_critic_new_params']
                instance.penalty2_critic.load_state_dict(penalty_critic_new_params['penalty_critic_new'])
                instance.penalty2_target_critic.load_state_dict(penalty_critic_new_params['target_critic_new'])
                instance.penalty2_critic_optimizer.load_state_dict(penalty_critic_new_params['penalty_critic_new_optimizer'])
        return instance
